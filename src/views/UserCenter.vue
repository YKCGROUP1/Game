  ...
  <div style="text-align: center;">
    ...
    <el-form :model="passForm" status-icon :rules="rules" ref="passForm" label-width="100px">
      ...
      <el-form-item>
        ...
        <el-button style="color: black !important;" @click="resetForm('passForm')">重置</el-button>
      </el-form-item>
    </el-form>
  </div>
    ...
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          const _this = this
          this.$axios.post('/api/updatePass', this.passForm).then(res => {

            _this.$alert(res.data.msg, '提示', {
              confirmButtonText: '确定',
              callback: action => {
                this.$refs[formName].resetFields();
                if(res.data.msg ==='修改成功,即将重新登录') {
                  localStorage.clear()
